...
**Fast path:** If detection found zero docs, papers, and images (code-only corpus), skip Part B entirely and go straight to Part C. AST handles code - there is nothing for semantic subagents to do. Write `graphify-out/.graphify_semantic.json` with:
...
After each subagent call completes, write its result to `graphify-out/.graphify_chunk_N.json`. Then inspect the tool result's `usage` field and mark that chunk explicitly:
...
new = json.loads(Path('graphify-out/.graphify_semantic_new.json').read_text()) if Path('graphify-out/.graphify_semantic_new.json').exists() else {'nodes':[],'edges':[],'hyperedges':[],'input_tokens':0,'output_tokens':0,'token_usage':{'status':'not-used','tracked_chunks':0,'untracked_chunks':0}}
