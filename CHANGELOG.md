...
## 0.8.50+vonderheidon.4 (2026-06-28)

- Feat: OpenCode Go is now the only automatically selected API backend. Graphify resolves its credential in this order: an explicit `api_key` argument, `OPENCODE_GO_API_KEY`, then OpenCode's global `$XDG_DATA_HOME/opencode/auth.json` or `~/.local/share/opencode/auth.json`. The global file must be a regular non-symlink, owned by the current user, with no group/other permissions. Missing credentials produce one login instruction: `opencode auth login --provider opencode-go`; malformed or unsafe auth files fail closed without exposing their contents.
- Security: credentials are consumed in memory only and never copied into repositories, shell profiles, graphs, reports, errors, or `cost.json`. Other providers, including Gemini, remain available only through explicit `--backend` selection.
- Docs: generated runbooks, including the Aider and Devin monoliths, now use global OpenCode Go auth for automatic semantic extraction and no longer suggest or install Gemini automatically. CLI help, README, generated artifacts, and skillgen drift guards reflect the new default.

- Fix: generated host-agent runbooks no longer record missing token metrics as a false `0 input / 0 output`. Chunks and runs now carry additive `token_usage` metadata (`complete`, `partial`, `unavailable`, `not-used`, or `legacy`) with tracked/untracked chunk counts; numeric fields remain known totals for compatibility. Reports and `cost.json` render unavailable or partial usage honestly, while old cost records remain readable as `legacy`. Gemini/native extraction remains fully tracked, and the Gemini extra guidance now installs into the same isolated `uv tool` environment.
- Fix: GraphML export now materializes hyperedges as collision-free auxiliary nodes, preserves their metadata and members, normalizes unsupported attributes, and writes atomically so a failed export cannot destroy an existing file. The exporter keeps the original NetworkX graph type and direction, strips internal attributes, and emits placeholders for missing hyperedge members.
