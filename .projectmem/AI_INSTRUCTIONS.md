...
<!-- >>> projectmem context layers >>> -->
## Context loading layers

- **short**: Low-token orientation to current project memory.
  CLI: `pjm brief`. MCP: `get_summary()`.
  Use for read-only orientation.
- **focused**: Token-budgeted context for a file, directory, or recent work.
  CLI: `pjm context --focus <path> --tokens <n>`. MCP: `get_context(tokens=<n>, focus=<path>)`.
  Use when a task needs targeted diagnostic context.
- **complete**: Full workflow rules, project memory, and structural map.
  CLI: `pjm instructions + pjm show + pjm map`. MCP: `get_instructions() + get_summary() + get_project_map()`.
  This layer is mandatory before any file or project memory mutation.

Read-only work can stop after the short or focused layer. This context-loading contract takes precedence over older session-start wording in this file.
<!-- <<< projectmem context layers <<< -->
...
- **pytest**: gotcha: local `python -m pytest` may use system Python 3.14 without project deps; verify ProjectMem with uv/Python 3.12 instead (from global-manual)
- **pytest**: Ran the focused pytest plus default Ruff cache; 57 tests passed, but Ruff failed before analysis while creating .ruff_cache. (from projectmem)
